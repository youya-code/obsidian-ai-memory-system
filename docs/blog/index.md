...
│                                               │
│  📁 知识笔记（学习沉淀，打项目标签）              │
│     ├── 项目架构与快速学习指南.md  [qwen-code]   │
│     ├── 二次开发完全指南.md        [qwen-code]   │
│     └── 终端多智能体协作原理.md     [qwen-code]   │
│                                               │
│  📁 记忆/（AI 工作记忆，按项目分文件夹）          │
...
│                                               │
│  📁 博客/（技术写作与分享）                      │
│     └── Claude-Code-Obsidian-AI-记忆系统.md     │
└──────────────────────────────────────────────┘
```

**关键设计**：知识与记忆分开存储。知识笔记是你在 Obsidian 里写的学习文档，按知识标签归类；记忆文件是 Claude 自动读写的项目约定和踩坑记录，按 `memory` 标签 + 项目文件夹隔离。两者共处一个 vault，但用途和读写方式完全不同。
